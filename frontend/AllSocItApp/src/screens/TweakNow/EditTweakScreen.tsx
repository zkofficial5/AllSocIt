...
import { tweakAPI, templateAPI } from "../../services/tweaknow";
...
type EditTweakScreenProps = {
  navigation: NativeStackNavigationProp<RootStackParamList, "EditTweak">;
  route: RouteProp<RootStackParamList, "EditTweak">;
};

export default function EditTweakScreen({
  navigation,
  route,
}: EditTweakScreenProps) {
  ...
  const loadTweak = async () => {
    try {
      const allTweaks = await tweakAPI.getAll(universeId);
      const tweak = allTweaks.find((t) => t.id === tweakId);
      if (tweak) {
        setContent(tweak.content);
        setImages(tweak.images || []);
        setCommentCount(tweak.comment_count.toString());
        setRetweetCount(tweak.retweet_count.toString());
        setQuoteCount(tweak.quote_count.toString());
        setLikeCount(tweak.like_count.toString());
        setViewCount(tweak.view_count.toString());
        setSourceLabel(tweak.source_label);
        setCustomDate(new Date(tweak.custom_date || tweak.created_at));
      } else {
        Alert.alert("Error", "Tweet not found");
        navigation.goBack();
      }
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    try {
      await tweakAPI.update(universeId, tweakId, {
      ...
      Alert.alert(
        "Error",
        error.response?.data?.detail || "Could not update tweak",
      );
    }
  };
        ...
        >
          <View
            style={[
              ...
              ((!content.trim() && images.length === 0) || saving) &&
